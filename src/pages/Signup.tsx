...
const Signup = () => {
  ...
  const planOptions = [
    { value: "starter", label: "$29 - " + t("pricing.starter") },
    { value: "growth", label: "$59 - " + t("pricing.growth") },
    { value: "agency", label: "$99 - " + t("pricing.agency") },
  ];
      ...
      <main className="pt-[72px] min-h-[calc(100vh-72px)] flex">
        <div className="hidden lg:flex lg:w-1/2 bg-primary flex-col justify-center px-16">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            <h2 className="font-heading text-[32px] text-white mb-2 leading-tight">{t("signup.panelTitle")}</h2>
            ...
            <div className="mt-12 p-6 rounded-xl bg-white/10 border border-white/20">
              ...
              <p className="font-body text-[15px] text-white/90 italic mb-4">"{t("signup.testimonial")}"</p>
              <p className="font-body text-[14px] text-white/70">{t("signup.testimonialAuthor")}</p>
            </div>
          </motion.div>
        </div>
        <div className="w-full lg:w-1/2 flex items-center justify-center px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-[440px] w-full py-8">
            <h1 className="font-heading text-[32px] md:text-[40px] text-secondary mb-2">{t("signup.getStarted")}</h1>
            <p className="font-body text-[15px] text-muted-foreground mb-8">{t("signup.createAccount")}</p>
            <div className="space-y-4">
              ...
              <div>
                <label className="font-body text-[14px] font-medium text-secondary block mb-1.5">{t("signup.password")} *</label>
                <input type="password" placeholder={t("signup.minPassword")} className="w-full px-4 py-3 rounded-lg border border-border font-body text-[16px] focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="font-body text-[14px] font-medium text-secondary block mb-1.5">{t("signup.company")}</label>
                  <input type="text" placeholder={t("signup.optional")} className="w-full px-4 py-3 rounded-lg border border-border font-body text-[16px] focus:outline-none focus:ring-2 focus:ring-primary" />
                </div>
                <div>
                  <label className="font-body text-[14px] font-medium text-secondary block mb-1.5">{t("signup.phone")}</label>
                  <input type="tel" placeholder={t("signup.optional")} className="w-full px-4 py-3 rounded-lg border border-border font-body text-[16px] focus:outline-none focus:ring-2 focus:ring-primary" />
                </div>
              </div>
              <div>
                <label className="font-body text-[14px] font-medium text-secondary block mb-1.5">{t("signup.plan")}</label>
                <div className="grid grid-cols-3 gap-2">
                  {planOptions.map((p) => (
                    <button key={p.value} type="button" onClick={() => setPlan(p.value)} className={`py-2.5 rounded-lg font-body text-[13px] font-medium transition-colors border ${plan === p.value ? "bg-primary text-white border-primary" : "border-border text-secondary hover:bg-muted"}`}>{p.label}</button>
                  ))}
                </div>
              </div>
            ...
            <p className="font-body text-[12px] text-muted-foreground mt-4 text-center">
              {t("signup.terms")}{" "}
              <a href="#" className="text-primary hover:underline">{t("signup.termsLink")}</a> {t("login.and")}{" "}
